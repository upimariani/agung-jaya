...
class mAnalisis extends CI_Model
{
    public function lap()
    {
        $data['analisis_jk'] = $this->db->query("SELECT COUNT(pelanggan.id_cust) as jml_cust, jk FROM `my_order` JOIN pelanggan ON my_order.id_cust=pelanggan.id_cust GROUP BY jk")->result();
    ...
    public function lap_harian_transaksi($tanggal, $bulan, $tahun)
    {
        ...
        $this->db->join('pengiriman', 'my_order.id_order = pengiriman.id_order', 'left');
        $this->db->where('status_order=4');
        $this->db->where('type_order=0');
